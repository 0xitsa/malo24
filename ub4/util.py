...
from typing import Any, ClassVar, Generic, Protocol, Self, TypeVarTuple, runtime_checkable

from typing_extensions import TypeVar

T, S, Args = TypeVar("T", infer_variance=True), TypeVar("S"), TypeVarTuple("Args")
...
class CustomTest(Generic[*Args, T]):
    ...
    _tests: ClassVar[list[CustomTest[*tuple[Any, ...], Any]]] = []
...
def custom_test(
    ...
    Use thiy by decorating the function you want to test with this and specify the input and expected output you want
    ...
    def inner(func: Callable[..., T]) -> Callable[..., T]:
        ...
        if not isinstance(input, tuple):
            input = (input,)
        CustomTest(input=input, output=output, name=name, func=func, _from_decorator=True)
        return func
